...
export default function Navbar() {
  return (
    <div>
      <div className="w-full h-[74px] flex items-center justify-center bg-amber-600 sm:h-25">
        <div className="w-[88%] h-12  flex bg-indigo-700 flex-row justify-between sm:h-[70px] xl:gap-[141px] ">
          <button className="w-8 h-8 bg-golden bg-[#C9A35B] self-center rounded-lg sm:w-12 sm:h-12 sm:rounded-xl xl:hidden"></button>
          ...
          <div className="hidden xl:flex xl:flex-row xl:bg-fuchsia-950 xl:gap-4">
            {navbarItems.map((item) => (
              <a
                key={item.id}
                href={item.link}
                className="hidden xl:text-white xl:text-base xl:mx-0 xl:px-0 xl:flex xl:items-center xl:justify-center "
              >
                {item.title}
              </a>
            ))}
          ...
          <div className="flex flex-row">
            ...
            <div className="w-12 h-12 bg-red-700 sm:w-[70px] sm:h-[70px]"></div>
          </div>
        </div>
      </div>
    </div>
